import { useForm } from 'react-hook-form'
import Button from '@components/Button'
import FormField from '@components/FormField'
import { exportPdfFromElement } from '@utils/pdf'

type Inputs = { name: string; currentStatus: string; newStatus: string }

export default function StatusChangeForm() {
  const { register, handleSubmit, watch } = useForm<Inputs>()
  const onSubmit = () => alert('保存しました（モック）')
  const previewId = 'status-change-preview'
  return (
    <div className="space-y-4">
      <h2 className="text-lg font-semibold">在留資格変更許可申請書</h2>
      <form onSubmit={handleSubmit(onSubmit)} className="grid sm:grid-cols-2 gap-4 bg-white p-4 rounded shadow">
        <FormField label="氏名"><input className="w-full border rounded px-3 py-2" {...register('name')} /></FormField>
        <FormField label="現在の在留資格"><input className="w-full border rounded px-3 py-2" {...register('currentStatus')} /></FormField>
        <FormField label="変更後の在留資格"><input className="w-full border rounded px-3 py-2" {...register('newStatus')} /></FormField>
        <div className="sm:col-span-2 flex gap-2">
          <Button type="button" variant="secondary" onClick={() => window.scrollTo({ top: document.body.scrollHeight, behavior: 'smooth' })}>プレビュー</Button>
          <Button type="button" onClick={() => exportPdfFromElement(previewId, 'status-change.pdf')}>PDF出力</Button>
          <Button type="submit" variant="secondary">保存</Button>
        </div>
      </form>
      <div id={previewId} className="bg-white p-4 rounded shadow">
        <div className="font-semibold mb-2">プレビュー</div>
        <div>氏名: {watch('name')}</div>
        <div>現在の在留資格: {watch('currentStatus')}</div>
        <div>変更後の在留資格: {watch('newStatus')}</div>
      </div>
    </div>
  )
}

